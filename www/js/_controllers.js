            ...
            if($scope.dev_option=="Delivery")
            {
              ...
              $scope.policy.forEach(function(e, i){
                if(e.policy_name==$scope.dev_option)
                {
                 ...
                 {
                   $scope.showMSg('Minimum order amount:'+min_amt);
                   return false;
                 }
                 else
                 {
...
basket.setPaymentOption(-1);
    $scope.confirmPlaceOrder=function()
    {
         ...
         if($scope.my.pre_order_delivery_time=="")
       {
       ...
       $ionicLoading.show({
            ...
            if($scope.payment_status==0)
            {
             ...
             $http.post(basket.API()+'Tigger.php?funId=12&rest_id='+basket.REST_ID()+'&user_id='+$scope.user.userid+'&order_policy_id='+basket.getOrderOption()+'&post_code='+$scope.user.postcode+'&OrderList='+JSON.stringify(basket.abcd())+'&address='+$scope.address+'&city='+$scope.user.city+'&payment_option='+basket.getPaymentOption()+'&total_amount='+$scope.total_amount+'&grand_total='+$scope.grand_amount+'&discount='+$scope.obtained_discount+'&offer_text='+$scope.offer_text+'&pre_order_delivery_time='+$scope.my.pre_order_delivery_time+'&comments='+$scope.my.comments+'&payment_status=0&paypal_transection_id='+$scope.paypal_transection_id+"&verification_code="+$scope.verification_code).success(function(data)
                {
...
if($scope.registration.country==null){
  ...
  /*Form validation end*/
    $ionicLoading.show({
      ...
      $http.post(basket.API()+'Tigger.php?funId=8&fname='+$scope.registration.fname+'&lname='+$scope.registration.lname+'&email='+$scope.registration.email+'&mobile_no='+$scope.registration.mobile+'&telephone_no='+$scope.registration.tel+'&postcode='+$scope.registration.postcode+'&address1='+$scope.registration.address1+'&address2='+$scope.registration.address2+'&city='+$scope.registration.city+'&country='+$scope.registration.country+'&password='+$scope.registration.password+"&dob_date="+$scope.dob+"&doa="+$scope.doa).success(function(data, status, headers, config)
      {
           $ionicLoading.hide();
           if (data.status=="Success")
           {
 ...
 temp+='<ul class="list">';
        ...
        /*        temp+='<li>';
             ...
             temp+=' <li>';
              ...
              temp+='</li>';*/
               temp+='<li>';
               temp+='<a class="item" menu-close nav-clear href="#/tab/login/7"><i class="ion-log-in"></i> Sign Out</a>';
              temp+='</li>';
            temp+='</ul>';
