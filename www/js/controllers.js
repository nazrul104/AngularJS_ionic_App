...
angular.module('ionicApp.controllers', [])
  ...
  .controller('NavCtrl', function($scope,$state, $ionicSideMenuDelegate, $ionicModal, basket) {
  ...
  $http.get(basket.API() + 'Tigger.php?funId=19&rest_id=' + basket.REST_ID(), {
    cache: true
  }).
  ...
  $scope.addFav = function(dish_id) {
    ...
    if (window.localStorage.getItem("UserDetails")) {
      ...
      success(function(data, status, headers, config) {
        ...
        title: 'Alert',
        cssClass: 'btnsubmit',
        template: 'This item already exists!'
      });
        }
      });
    } else {
  ...
  if (window.localStorage.getItem("schedule")) {
    ...
    var weekday_id = currentdate.getDay() + 1;
    ...
    for (var i = 0; i < $scope.schedule.schedule.length; i++) {
      ...
      if ($scope.schedule.schedule[i]['weekday_id'] == weekday_id) {
        if ($scope.schedule.schedule[i]['list'].length == 1) {
          ...
          $http.post(basket.API() + "Tigger.php?funId=42&opening=" + opening + "&closing=" + closing + "&shift=" + basket.SHIFT()).
          success(function(data, status, headers, config) {
            if (data.status == 0) {
              basket.setIsRestuarentOpen(false);
              basket.setRestaurantOpenStatusString(data.msg);
            }
  ...
  .controller('cart', function($scope,$cordovaInAppBrowser, $state, $filter, $rootScope, $http, $ionicPopup, $stateParams, $ionicModal, $timeout, $ionicSlideBoxDelegate, $location, $ionicLoading, basket, $ionicSideMenuDelegate) {
  var currentdate = new Date();
  ...
  $scope.openstatusstr = "";
  $scope.isDeliverable = 1;


         if(window.localStorage.getItem("minimum_order"))
         {
           $scope.min_order = window.localStorage.getItem("minimum_order");
         }



  if (basket.GetDiscount() > 0) {
    $scope.grand_amount = (basket.cartTotalAmount() - basket.GetDiscount()).toFixed(2);
  } else {
    $scope.grand_amount = basket.cartTotalAmount().toFixed(2);
  }

  if ($scope.isRestuarentOpen == false) {
    $scope.openstatusstr = basket.getRestaurantOpenStatusString();
  }
    ...
    $scope.policy.forEach(function(e, i) {
      if (e.policy_id == policy_id) {
        $scope.min_order = e.min_order;
         window.localStorage.setItem("minimum_order", $scope.min_order);
      }
    });
    //$scope.scheduleProcess();
  }

  $scope.confirmOrderOPtion = function() {
      ...
      if ($scope.dev_option == "Delivery") {
        ...
        $scope.policy.forEach(function(e, i) {
          if (e.policy_name == $scope.dev_option) {
            var min_amt = parseFloat(e.min_order);
    ...
    for (var i = 0; i < $scope.schedule.schedule.length; i++) {
      ...
      if ($scope.schedule.schedule[i]['weekday_id'] == weekday_id) {
        if ($scope.schedule.schedule[i]['list'].length == 1) {
          ...
          $scope.selected_policy = window.localStorage.getItem("selected_policy");
          $scope.user = JSON.parse(window.localStorage.getItem("UserDetails"));
          $ionicLoading.show({
            template: 'Loading...'
          });
          $http.post(basket.API() + 'Tigger.php?funId=41&rest_id=' + basket.REST_ID() + '&user_id=' + $scope.user.userid + "&opening=" + opening + "&closing=" + closing + "&policy_time=" + $scope.policy_time + "&order_option=" + $scope.selected_policy + "&shift=" + basket.SHIFT()).
          success(function(data, status, headers, config) {
            ...
            if (data.status == 0) {
              ...
              basket.setIsRestuarentOpen(false);
              $scope.PreOrderTime = data;
              $scope.availabletime=data.delivery_time;
          


              basket.setPreorderTime($scope.availabletime);
              if (data.delivery_area) {

                if (data.delivery_area.app[0].status == "Failed") /*If restaurant cannot delivery to this area*/
                {
                   ...
                   $location.path('/tab/cart');
                                 $ionicPopup.alert({
                                    title: 'Alert',
                                    cssClass:'btnsubmit',
                                    template: "Sorry! We do not deliver to this area of postcode, however you may call us or collect your order"
                                });
            ...
            /*end*/
            /************************###############**********************/
            /*****#####If restaurant is Opened#######*****/
            /********************************************************/

            if (data.status == 1) {
              basket.setDeliveryTime(data.delivery_time);
              if (data.delivery_area) {
                if (data.delivery_area.app[0].status == "Failed") /*If restaurant cannot delivery to this area*/
                {
                   $location.path('/tab/cart');
        ...
        if ($scope.schedule.schedule[i]['list'].length == 2) {
          ...
          $scope.policy_time = window.localStorage.getItem("selected_policy_time");
          $http.post(basket.CMS_API() + "config.php?x=TimeProcess2&opening1=" + opening1 + "&opening2=" + opening2 + "&closing1=" + closing1 + "&closing2=" + closing2 + "&current_time=" + $scope.generateTimeFormate() + "&policy_time=" + $scope.policy_time).
          success(function(data, status, headers, config) {
            if (data != "false") {
  ...
  $scope.check_verification_code = function() {
    ...
    var mypop = $ionicPopup.prompt({
      title: 'Verify Code',
      template: 'You have been sent SMS with verification code.',
      inputType: 'number',
  ...
  $scope.confirmPlaceOrder = function() 
  {
    ...
    if ($scope.payment_status == 0) {
      ...
      $http.post(basket.API() + "Tigger.php?funId=12&rest_id=" + basket.REST_ID() + "&user_id=" + $scope.user.userid + "&order_policy_id=" + basket.getOrderOption() + "&post_code=" + $scope.user.postcode + "&OrderList=" + JSON.stringify(basket.abcd()) + "&address=" + $scope.user.address1 + "&city=" + $scope.user.town + "&payment_option=" + basket.getPaymentOption() + "&total_amount=" + $scope.total_amount + "&grand_total=" + $scope.grand_amount + "&discount=" + $scope.obtained_discount + "&offer_text=" + $scope.offer_text + "&pre_order_delivery_time=" + $scope.my.devtime + "&comments=" + $scope.my.comments + "&payment_status=0&paypal_transection_id=0&verification_code=" + $scope.verification_code + "&platform=" + basket.Deviceplatform()).success(function(data) {
        ...
        if (data.status == "Success") {
          ...
          window.localStorage.removeItem("selected_policy_time");
          window.localStorage.removeItem("selected_policy_id");
    ...
    if ($scope.payment_status == 1) {
      basket.setPayPal_pending_status(true);
      basket.setComments($scope.my.comments);
      app.doAction();
    }
  }
  ...
  if (basket.getPreorderTime() != "") {
    $scope.availabletime = basket.getPreorderTime();
  }
  ...
  .controller('PayPalSuccess', function($scope, $http, $ionicModal, basket, $ionicPopup, $location) {
  $scope.verification_code = 0;
  $scope.vcode = 0;
  $scope.testvcode = 0;
  $scope.check_verification_code = function() {
    var mypop = $ionicPopup.prompt({
      title: 'Verify Code',
      template: 'Enter verification code' + $scope.testvcode,
      inputType: 'number',
      inputPlaceholder: ''
    }).then(function(res) {
      if (res != undefined && res != "") {
  ...
  $scope.confirmPlaceOrder = function() {
    ...
    $scope.user = JSON.parse(window.localStorage.getItem("UserDetails"));
    $scope.payment_status = basket.getPaymentOption();
    $http.post(basket.API() + "Tigger.php?funId=12&rest_id=" + basket.REST_ID() + "&user_id=" + $scope.user.userid + "&order_policy_id=" + basket.getOrderOption() + "&post_code=" + $scope.user.postcode + "&OrderList=" + JSON.stringify(basket.abcd()) + "&address=" + $scope.user.address1 + "&city=" + $scope.user.town + "&payment_option=" + basket.getPaymentOption() + "&total_amount=" + $scope.total_amount + "&grand_total=" + $scope.grand_amount + "&discount=" + $scope.discount + "&offer_text=" + $scope.offer_text + "&devtime=" + basket.getDeliveryTime() + "&comments=" + basket.getComments() + "&payment_status=1&paypal_transection_id=0&verification_code=" + $scope.verification_code + "&platform=" + basket.Deviceplatform()).success(function(data) {
      if (data.status == "sms_sent") {
        $scope.vcode = data.code;
        $scope.testvcode = data.code;
        $scope.check_verification_code();
  ...
  /*  if (basket.getPayPal_pending_status()==true)
   {
  ...
  $scope.checkString = function(myData) {
    ...
    var stringOnly = /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/i;
  ...
  $scope.registration = function() {
    ...
    /*    if($scope.registration.email!=null)
    {
     $scope.checkString($scope.registration.email);
    }*/
...
*/
  ...
  $scope.removeFavItem = function(pid)
  {
      $http.get(basket.API() + 'Tigger.php?funId=39&target_id=' + pid).success(function(data, status, headers, config) {
        console.log(data);
     $scope.FaVlist();
  }).
  error(function(data, status, headers, config) {
  ...
  $http.get(basket.CMS_API() + 'config.php?x=ContactPage', {
    cache: true
  }).
  success(function(data, status, headers, config) {
    // $scope.data=data;     
  ...
  $scope.addReservation = function() {
    ...
    /*    if($scope.res.email!=null)
                {
                 $scope.checkString($scope.res.email);  
                }*/
